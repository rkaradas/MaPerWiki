...
export default class SearchResultsController
{
  ...
  searchInWiki(data)
  {
    selfInstance.viewModel.model.isBusy = true;
    this.searchHelper.searchInWiki(data, (el, err)=> this.searchCallback(el,err), (el,first,err)=>this.searchAddEntryCallback(el,first,err));
  }
  searchAddEntryCallback(el,first,err)
  {
    if(err){
      ...
      if(first){
        ...
        selfInstance.viewModel.model.results.splice(0,1,el);
      }
      selfInstance.viewModel.model.results[0].fileEntries.sort(function(a, b){
  ...
  searchCallback(el, err)
  {
    ...
    selfInstance.viewModel.model.isBusy = false;
  }


  abortSearch()
  {
    if(selfInstance.searchHelper.currentSearchPromise)
    {
      selfInstance.searchHelper.currentSearchPromise.cancel();
      selfInstance.viewModel.model.isBusy = false;
    }
  }
