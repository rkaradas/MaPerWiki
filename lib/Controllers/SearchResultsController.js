  ...
  Init()
  {
    ...
    this.vue = new Vue({
      ...
      methods: {
        ...
        clearResults()
        {
          selfInstance.vue.$data.model.results = [];
        },
        removeEntry(fileEntry)
        {
          var idxOfFileEntry;
          if( (idxOfFileEntry = selfInstance.vue.$data.model.results.indexOf(fileEntry)) != -1 )
          {
            selfInstance.vue.$data.model.results.splice(idxOfFileEntry, 1);
          }
        }
      }
    });
