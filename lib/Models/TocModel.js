...
export default class TocModel
{
  ...
  tryGetSetextHeader(editor, row)
  {
    var possibleHeaderText = editor.lineTextForBufferRow(row);
    var possibleSetext = editor.lineTextForBufferRow(row+1);
    if(possibleSetext){
      var isH1 = possibleSetext.match(/^=+ *$/g);
      var isH2 = possibleSetext.match(/^-+ *$/g);
      if(isH1 || isH2){
        var header = [""];
        header.push(isH1?"#":"##");
        header.push(possibleHeaderText);
        return header;
      }
  ...
  updateData(editor, vueData)
  {
    ...
    */
    ...
    for (var row = 0; row < lineCount; row++)
    {
      ...
      if(!isCodeBlockStart)
      {
        isCodeBlockStart = lineText.match(/^(`{3})(.*)/);
      }
      else if(isCodeBlockStart && !isCodeBlockEnd)
      {
        isCodeBlockEnd = lineText.match(/^(`{3})(.*)/);
        if(isCodeBlockEnd){
          isCodeBlockStart = false;
          isCodeBlockEnd = false;
        }

      }
      console.log(isCodeBlockStart + " - " + lineText);

      if(lineText && !isCodeBlockStart){
        var header = lineText.match(/^(#{1,6}) +([^#]{1}[\S]{1}.*) */); // string must begin with # for 1 to 6 times
        // if not atx header, we have to check if it's setext header.
        if(!header)
        {
          header = this.tryGetSetextHeader(editor, row);
        }
