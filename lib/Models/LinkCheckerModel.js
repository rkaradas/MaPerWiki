...
export default class LinkCheckerModel
{
  ...
  findReferenceInfiles()
  {
    ...
    var retPromise = atom.workspace.scan(new RegExp(selfInstance.utils.markdownLinkRegexPattern, 'g'),{paths: globPattern/*this.files*/, onPathsSearched},function(result,err){
      ...
      result.matches.forEach((itm,idx)=>{
        var ms = selfInstance.utils.tryParseMarkdownLink(itm.matchText);
        ...
        // check if is url
        var mIsUrl = selfInstance.utils.tryParseWebUrl(ms[2]);
  ...
  findAllHeaders(file)
  {
    ...
    //file.read(true).then((content)=>{
      ...
      if(!lines) return; // content is empty
