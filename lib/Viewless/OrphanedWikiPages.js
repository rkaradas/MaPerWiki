...
export default class OrphanedWikiPages
{
  ...
  */
  ...
  markAllOrphanedLinks()
  {
    ...
    var joinedPath = path.join(projectPath[0], wikiFilePath);
    ...
    leftPanels.forEach((itm, idx)=>{
      ...
      try{
        if(selfInstance.projectMarkdownFiles)
        {
          ...
          {
            selfInstance.projectMarkdownFiles.forEach((fPath,pathIdx)=>{

              var curTEEntry = itm.item.entryForPath(fPath);

              if(selfInstance.foundPathsInWikiLinks.indexOf(path) < 0 && fPath != normalizedFullWikiFilePath)
              {
